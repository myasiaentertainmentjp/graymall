...
export default function BannerCarousel({ slides = defaultSlides, autoPlayInterval = 6000 }: Props) {
  ...
  const renderSlideContent = (slide: BannerSlide) => (
    <div className="relative w-full h-[180px] sm:h-[220px] lg:h-[280px] overflow-hidden rounded-xl">
      <Image
        src={slide.image}
        alt={slide.alt}
        fill
  ...
  return (
    ...
    >
      ...
      >
        ...
        <button
          onClick={goToPrev}
          className="absolute left-2 top-1/2 -translate-y-1/2 w-10 h-10 bg-black/60 hover:bg-black/80 rounded-full flex items-center justify-center text-white transition-colors z-10"
          aria-label="前のスライド"
        >
        ...
        <button
          onClick={goToNext}
          className="absolute right-2 top-1/2 -translate-y-1/2 w-10 h-10 bg-black/60 hover:bg-black/80 rounded-full flex items-center justify-center text-white transition-colors z-10"
          aria-label="次のスライド"
        >
      ...
      <div className="hidden lg:block relative">
        <div className="grid grid-cols-3 gap-4">
          ...
          <div
            ...
            onClick={goToPrev}
          >
            {renderSlideContent(slides[getSlideIndex(-1)])}
          </div>

          {/* 中央スライド */}
          <div>
            {slides[currentIndex].link ? (
              <Link href={slides[currentIndex].link} className="block">
        ...
        <button
          onClick={goToPrev}
          className="absolute left-2 top-1/2 -translate-y-1/2 w-12 h-12 bg-white/90 hover:bg-white rounded-full flex items-center justify-center text-gray-800 shadow-lg transition-colors z-20"
          aria-label="前のスライド"
        >
          <ChevronLeft className="w-6 h-6" />
        </button>
        <button
          onClick={goToNext}
          className="absolute right-2 top-1/2 -translate-y-1/2 w-12 h-12 bg-white/90 hover:bg-white rounded-full flex items-center justify-center text-gray-800 shadow-lg transition-colors z-20"
          aria-label="次のスライド"
        >
          <ChevronRight className="w-6 h-6" />
        </button>

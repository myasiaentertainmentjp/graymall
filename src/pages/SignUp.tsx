    ...
    <div className="min-h-screen bg-gray-50 flex items-center justify-center px-4">
      <div className="max-w-md w-full bg-white rounded-lg shadow-md p-8">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-600 mt-3">新規登録</p>
        </div>
        ...
        {!success && (
          <>
            ...
            >
              ...
              <span>{googleLoading ? '接続中...' : 'Googleで登録'}</span>
            </button>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          >
            {loading ? '登録中...' : success ? '登録完了！' : 'メールアドレスで登録'}
          </button>
        </form>
        ...
        {!success && (
          <div className="mt-6 text-center text-sm text-gray-600">
            すでにアカウントをお持ちの方は{' '}
            <Link to={`/signin${redirectTo !== '/' ? `?redirect=${encodeURIComponent(redirectTo)}` : ''}`} className="text-blue-600 hover:underline">
              サインイン
            </Link>
          </div>
        )}

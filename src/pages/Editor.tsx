...
type AffiliateRate = 0 | 10 | 20 | 30 | 40 | 50;
  ...
  export default function Editor() {
    ...
    // 自動保存（リトライ付き）
    async function autoSave(retry = 0) {
      ...
      if (result) {
        ...
        // 失敗時: 5秒後にリトライ（最大2回）
        setTimeout(() => {
          if (!savingRef.current && !autoSavingRef.current) {
            autoSave(retry + 1);
          }
        }, 5000);
      } else {
        setSaveError('自動保存に失敗しました。ネットワークを確認してください。');
      }
    }
    ...
    useEffect(() => {
      const timer = setInterval(() => {
        if (!savingRef.current && !autoSavingRef.current && status === 'draft') {
          autoSave();
        }
      }, 10000);

      return () => clearInterval(timer);
    }, [title, content, paidContent, status]);

    // デバウンス自動保存（入力停止3秒後に発火）
    useEffect(() => {
    ...
    }, [title, content, paidContent]);

    // ページ離脱時に保存を試みる
    useEffect(() => {
      const handleBeforeUnload = (e: BeforeUnloadEvent) => {
        const currentContent = JSON.stringify({ title, content, paidContent });
        if (currentContent !== lastSavedContentRef.current && status === 'draft') {
          // navigator.sendBeaconは使えないのでsync XMLHttpRequestでベストエフォート
          e.preventDefault();
          e.returnValue = '';
        }
      };
      ...
      <div className="min-h-screen bg-white">
        <div className="border-b border-gray-200 bg-white sticky top-0 z-40">
          <div className="max-w-[1400px] mx-auto px-4 h-14 flex items-center justify-between gap-3">
            ...
            {/* 自動保存ステータス */}
            {(autoSaving || lastSavedAt || saveError) && (
              <div className={`hidden sm:block text-xs ${saveError ? 'text-red-500' : 'text-gray-400'}`}>
                {saveError ? (
                  <button onClick={() => { setSaveError(null); autoSave(); }} className="underline hover:text-red-700">
                    {saveError}（再試行）
                  </button>
                ) : autoSaving ? (
                  '自動保存中...'

...
- **スタイル**: ZOZO/tips風（中央1枚 + 左右チラ見せ）
- **PC表示**: 中央60% + 左右18%ずつ
- **モバイル**: 1枚表示 + スワイプ対応
- **自動切替**: 6秒間隔
- **画像サイズ**: 1500×600px（推奨）/ 1200×480px（アスペクト比 2.5:1）
- **保存場所**: `/public/banners/banner-1.png` 〜 `banner-4.png`
- **設定箇所**: `BannerCarousel.tsx` の `defaultSlides` 配列でリンク先・タイトル変更可能
